...
NEXTCLOUD_HOME=${DEPLOY_ROOT}/${NEXTCLOUD_DIR}
...
deploy_nextcloud()
{
    ...
    then
        ...
        exit 1
    elif [ -d ${NEXTCLOUD_HOME} ]
    then
        echoY "Already deployed nextcloud in ${NEXTCLOUD_HOME}, check it there first!"
        exit 1
    else
        echoY "Deploying nextcloud..."
        cp -a ./src ${NEXTCLOUD_HOME}
        echoG "nextcloud has been deployed to ${NEXTCLOUD_HOME} successfully."
        cat ./src/README.md
    fi
}
